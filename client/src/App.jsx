...
import { METRICS, ARR_BANDS, ACV_BANDS, PRICING_MODELS, FUNDING_STAGES } from './data/benchmarks.js'
...
export default function App() {
  ...
  function buildShareUrl() {
    const state = btoa(JSON.stringify({ arrBand, acvBand, pricingModel, fundingStage, userVals, iconiqVals, additionalContext }))
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1.25rem 4rem' }}>
      ...
      <section style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end' }}>
            <div style={{ fontSize: '0.75rem', color: filledCount > 0 ? 'var(--accent)' : 'var(--text-3)', paddingBottom: 8 }}>
              {filledCount} / {totalMetrics} metrics entered
            </div>
          </div>
        </div>
      </section>
      ...
      <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', marginBottom: '1.5rem', fontSize: '0.72rem', color: 'var(--text-3)', alignItems: 'center' }}>
        {[
          { color: '#378ADD', label: 'Benchmark.it (ARR)' },
          { color: '#4ade80', label: 'High Alpha (ARR)' },
          { color: '#a78bfa', label: 'Benchmark.it (ACV)' },
          { color: '#fbbf24', label: 'Benchmark.it (pricing)' },
      ...
      <CollapsibleSection title="Methodology &amp; data sources">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 16 }}>
          {[
            { name: 'Benchmark.it — 2024 SaaS Benchmarks', detail: 'Based on 583 participant companies. Covers CY-24 data. Provides P25/Median/P75 segmented by ARR band, ACV band, and pricing model. See the report for more information on samples per metric.', url: 'https://www.benchmarkit.ai/2025benchmarks' },
      ...
      <footer style={{ marginTop: '2rem', paddingTop: '1.5rem', borderTop: '1px solid var(--border)', fontSize: '0.68rem', color: 'var(--text-3)', lineHeight: 1.7 }}>
        <p>Data: Benchmark.it 2024 · High Alpha 2025 · ICONIQ Growth 2025 · Serena 2026</p>
